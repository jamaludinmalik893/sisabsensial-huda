...
-- Update wali_kelas foreign key setelah tabel kelas ada
ALTER TABLE guru ADD CONSTRAINT fk_guru_wali_kelas 
FOREIGN KEY (wali_kelas) REFERENCES kelas(id_kelas) ON DELETE SET NULL;
...
-- Data Guru (password: 'root' untuk semua)
INSERT INTO guru (nip, nama_lengkap, email, password, nomor_telepon, status, alamat) VALUES 
('196801011990011001', 'Drs. Ahmad Wijaya, M.Pd', 'ahmad.wijaya@smkalhuda.sch.id', 'root', '081234567890', 'admin', 'Jl. Veteran No. 123, Kediri'),
('197205151998022002', 'Sri Mulyati, S.Kom', 'sri.mulyati@smkalhuda.sch.id', 'root', '081234567891', 'guru', 'Jl. Hayam Wuruk No. 45, Kediri'),
('198003102005011003', 'Bambang Sutrisno, S.T', 'bambang.sutrisno@smkalhuda.sch.id', 'root', '081234567892', 'guru', 'Jl. Brawijaya No. 78, Kediri'),
('198712252010012004', 'Indah Permata, S.Pd', 'indah.permata@smkalhuda.sch.id', 'root', '081234567893', 'guru', 'Jl. Diponegoro No. 56, Kediri')
...
SELECT 
    '012345678' || LPAD(generate_series::text, 2, '0') as nisn,
    CASE 
        WHEN generate_series % 4 = 1 THEN 'Ahmad Rizki Pratama ' || generate_series
        WHEN generate_series % 4 = 2 THEN 'Siti Nurhaliza ' || generate_series
        WHEN generate_series % 4 = 3 THEN 'Budi Santoso ' || generate_series
        ELSE 'Dewi Sartika ' || generate_series
    END as nama_lengkap,
...
-- Sample Jurnal Harian
INSERT INTO jurnal_harian (id_guru, id_mapel, id_kelas, tanggal_pelajaran, waktu_mulai, waktu_selesai, judul_materi, materi_diajarkan)
SELECT 
    (SELECT id_guru FROM guru WHERE nip = '197205151998022002' LIMIT 1),
    (SELECT id_mapel FROM mata_pelajaran WHERE nama_mapel = 'Pemrograman Web' LIMIT 1),
    (SELECT id_kelas FROM kelas WHERE nama_kelas = 'X RPL 1' LIMIT 1),
    CURRENT_DATE,
    '08:00:00',
    '09:30:00',
    'Pengenalan HTML dan CSS',
    'Pembelajaran dasar HTML dan CSS untuk membuat halaman web sederhana'
WHERE NOT EXISTS (
    SELECT 1 FROM jurnal_harian 
    WHERE tanggal_pelajaran = CURRENT_DATE 
    AND id_guru = (SELECT id_guru FROM guru WHERE nip = '197205151998022002' LIMIT 1)
);
